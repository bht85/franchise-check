...
export default function PremiumReportView({ brandName }: { brandName: string }) {
  return (
    <div className="w-full mb-8 relative">
      {/* ── 프리미엄 배너/헤더 ── */}
      <div className="bg-slate-900 rounded-t-3xl p-8 relative overflow-hidden text-white border border-slate-800">
        ...
        <div className="flex items-center gap-3 mb-4 relative z-10">
          ...
          <span className="text-slate-400 text-sm font-semibold tracking-wide">AI 서류 정밀 분석 리포트</span>
        </div>
        
        <h1 className="text-2xl font-black mb-2 relative z-10 leading-snug">
          {brandName} <span className="text-slate-300">계약서 및 정보공개서</span><br />
          법률 리스크 스캔 결과
        </h1>
        
        <p className="text-slate-400 text-sm mb-6 max-w-[90%] relative z-10">
          총 142개 조항을 스캔하여 독소조항 및 본사 구두 설명과의 불일치 항목을 찾아냈습니다.
        </p>

        <div className="flex gap-3 text-sm font-medium relative z-10">
          <div className="bg-slate-800/80 rounded-lg px-3 py-2 flex items-center gap-2 border border-slate-700/50">
      ...
      {/* ── 심층 분석 내용 ── */}
      <div className="bg-white rounded-b-3xl border-x border-b border-slate-200 p-6 sm:p-8 shadow-sm">
        
        <div className="flex items-center gap-2 mb-6">
          <ShieldAlert className="text-red-600" size={24} />
          <h2 className="text-lg font-black text-slate-900">명백한 불일치 (거짓말 탐지) 2건</h2>
        </div>
          ...
          <div className="p-5">
            <div className="flex gap-4 mb-5">
              <div className="flex-1 bg-slate-50 rounded-xl p-4 border border-slate-200 relative">
                <p className="text-xs text-slate-500 font-bold mb-1">본사 안내 (설문 답변)</p>
                <p className="text-slate-900 font-medium">"우리는 로열티가 전혀 없습니다."</p>
                <div className="absolute top-1/2 -right-3 -translate-y-1/2 w-6 h-6 bg-white rounded-full border border-slate-200 flex items-center justify-center shadow-sm z-10 text-slate-400 font-bold text-xs">VS</div>
              </div>
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-2xl p-6 relative overflow-hidden">
          ...
          <div className="bg-white rounded-xl p-4 border border-blue-100 shadow-sm relative z-10 font-mono text-sm text-slate-700 whitespace-pre-wrap leading-relaxed">
        ...
        {/* ── 액션 ── */}
        <button className="w-full mt-6 bg-slate-900 text-white font-bold py-4 rounded-xl flex items-center justify-center gap-2 hover:bg-slate-800 transition-colors shadow-lg">
          <Download size={18} />
          전문가용 리포트 PDF 다운로드
        </button>
